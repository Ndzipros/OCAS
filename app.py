...
def dashboard():
    ...
    if DATABASE_URL:
        emergency   = db_count("SELECT COUNT(*) FROM alerts WHERE priority = :1", ("Emergency",))
        warning     = db_count("SELECT COUNT(*) FROM alerts WHERE priority = :1", ("Warning",))
        info        = db_count("SELECT COUNT(*) FROM alerts WHERE priority = :1", ("Information",))
    else:
        emergency   = db_count("SELECT COUNT(*) FROM alerts WHERE priority = ?", ("Emergency",))
        warning     = db_count("SELECT COUNT(*) FROM alerts WHERE priority = ?", ("Warning",))
        info        = db_count("SELECT COUNT(*) FROM alerts WHERE priority = ?", ("Information",))
...
def create_alert():
    if request.method == "POST":
        if DATABASE_URL:
            db_execute(
                "INSERT INTO alerts (title, message, priority) VALUES (:1, :2, :3)",
...
def delete_alert(alert_id):
    if DATABASE_URL:
        db_execute("DELETE FROM alerts WHERE id = :1", (alert_id,))
    else:
...
def public_feed():
    ...
    return render_template("feed.html", alerts=alerts,
